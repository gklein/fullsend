...
func TestSubmitFormalReview_EmptyBody(t *testing.T) {
	fc := forge.NewFakeClient()
	fc.AuthenticatedUser = "fullsend-bot"
	printer := ui.New(io.Discard)
	...
	require.NoError(t, err)
	require.Len(t, fc.CreatedReviews, 1)
	assert.Empty(t, fc.CreatedReviews[0].Body, "formal review body should be empty to avoid duplicate notifications")
}
