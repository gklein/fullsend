...
func TestPrioritizeSchedulerWorkflowContent(t *testing.T) {
	...
	require.NoError(t, err)
	s := string(content)
	assert.NotContains(t, s, "schedule:", "cron trigger should be disabled by default (#778)")
	assert.NotContains(t, s, "cron:", "cron trigger should be disabled by default (#778)")
	assert.Contains(t, s, "workflow_dispatch")
