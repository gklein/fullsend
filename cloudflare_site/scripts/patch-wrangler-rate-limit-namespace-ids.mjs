#!/usr/bin/env node
/**
 * CI: set Workers ratelimit namespace_id to base + offset(CLOUDFLARE_PROJECT_NAME).
 * Local dev uses committed bases in wrangler.toml; this script is not run for wrangler dev.
 *
...
console.log(
  `patched ratelimit namespace_id: OAUTH=${oauthId} USER=${userId} (offset=${offset.toString()} from CLOUDFLARE_PROJECT_NAME)`,
);
