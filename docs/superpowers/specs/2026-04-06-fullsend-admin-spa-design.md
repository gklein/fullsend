...
| Organizations for the signed-in user (admin org list) | `GET https://api.github.com/user/orgs` (browser / Octokit `paginate`) | Bearer user token; SPA calls GitHub directly. Prefer **`/user/orgs`** over **`/user/memberships/orgs`**: GitHub App user tokens often return **no rows** from the memberships endpoint while **`/user/orgs`** lists orgs the token may access. **Classic OAuth:** `read:org` for private org visibility. **GitHub App user-to-server tokens:** org access follows **App installation permissions** (see GitHub REST docs). |
