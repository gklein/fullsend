...
| Organizations for the admin picker (step toward repos) | `GET https://api.github.com/user/repos` (browser / Octokit `paginate`) with `affiliation=owner,collaborator,organization_member`; unique `owner.login` where `owner.type` is **Organization** | Bearer user token. Org names are **not** from membership or `GET /user/orgs`; they are inferred from **repositories the token can access**, matching a repo-first workflow. **Classic OAuth:** private repos typically need **`repo`** (or **`public_repo`** for public only) on `GET /user/repos`. **GitHub App user-to-server tokens:** follow repository (and installation) permissions for each repo returned. |
| Read-only layer analyze — repository exists | `GET https://api.github.com/repos/{owner}/{repo}` | Bearer token. **404** means the repo (e.g. org `.fullsend`) does not exist. Used by the config-repo layer via Octokit `repos.get`. |
| Read-only layer analyze — file body | `GET https://api.github.com/repos/{owner}/{repo}/contents/{path}` | JSON includes base64 `content` for files. **404** when the path is missing. Used by config-repo (`config.yaml`), workflows (managed paths under `.fullsend`), and enrollment (shim workflow in each enabled repo). |
| Read-only Actions — repository secret metadata | `GET https://api.github.com/repos/{owner}/{repo}/actions/secrets/{secret_name}` | Confirms a secret **name** exists without returning the value. **404** when absent. Secrets layer on `.fullsend`. |
| Read-only Actions — repository variable | `GET https://api.github.com/repos/{owner}/{repo}/actions/variables/{name}` | **404** when absent. Secrets layer uses HTTP status only for presence (response may include a sensitive `value` field — do not log or display it). |
| Read-only Actions — org secret metadata | `GET https://api.github.com/orgs/{org}/actions/secrets/{secret_name}` | Dispatch-token layer. Typically requires **`admin:org`** (or an org-role token that can read org secrets). **403** is surfaced as **unknown** (“cannot verify”) in the SPA, distinct from **404** (secret missing). |
| Optional OAuth scope preflight | `HEAD https://api.github.com/user` | Response header **`X-OAuth-Scopes`**: comma-separated classic OAuth scopes. Empty or absent header → SPA treats scope comparison as **skipped** (matches `internal/layers/preflight.go` behavior for GitHub App user tokens and fine-grained PATs). |
